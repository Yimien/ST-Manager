...
export default function presetGrid() {
  return {
    ...
    selectPresetItem(item, type, shouldScroll = false) {
      ...
      if (shouldScroll && item) {
        this.$nextTick(() => {
          const domId =
            type === "prompt"
              ? `preset-prompt-${item.key}`
              : `preset-regex-${item.key || "unknown"}`;
          if (item.key) {
    ...
    get filteredPresetItems() {
      ...
      if (this.uiPresetFilter === "enabled")
        return prompts.filter((p) => p.enabled);
      if (this.uiPresetFilter === "disabled")
        return prompts.filter((p) => !p.enabled);
    ...
    createSnapshot(type) {
      // 触发快照功能
      window.dispatchEvent(
        new CustomEvent("create-snapshot", {
    ...
    openBackupFolder(type) {
      // 打开备份文件夹
      window.dispatchEvent(
        new CustomEvent("open-backup-folder", {
          detail: { type },
        }),
      );
    },
    ...
    editPresetRawFromDetail() {
      ...
      window.dispatchEvent(
        new CustomEvent("open-script-file-editor", {
          detail: {
    ...
    collectPromptMeta(prompt) {
      ...
      pushMeta(
        "position",
        prompt.position || prompt.insertion_position || prompt.insert_position,
      );
    ...
    normalizePrompts(list) {
      ...
      return list.map((prompt, idx) => {
        ...
        if (disabledRaw !== undefined && disabledRaw !== null) {
          ...
          enabledRaw !== null &&
          !(typeof enabledRaw === "string" && enabledRaw.trim() === "")
        ) {
